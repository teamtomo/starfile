...
class StarParser:
    """Parser for STAR file format data blocks."""

    filename: Path
    n_lines_in_file: int
    n_blocks_to_read: int | None
    ...
    def __init__(
        self,
        filename: PathLike,
        n_blocks_to_read: int | None = None,
        ...
        self.parse_as_string = parse_as_string if parse_as_string is not None else []
    ...
    def parse_file(self) -> None:
        """Parse the STAR file into data blocks."""
    ...
    def _parse_data_block(self) -> tuple[str, DataBlock]:
        """Parse a data block header and return block name and data."""
        ...
        msg = "Unexpected end of file while parsing data block"
        raise ValueError(msg)

    def _parse_simple_block(self) -> dict[str, str | int | float]:
        """Parse a simple (key-value) data block."""
        block: dict[str, str | int | float] = {}
        while self.current_line_number <= self.n_lines_in_file:
            if self.current_line.startswith("data"):
                ...
                column_name = k[1:]
                # NOTE: Why casing as an "any" here?
                parse_column_as_string = any(
                    column_name == col for col in self.parse_as_string
                )
                if parse_column_as_string:
                    block[column_name] = v
                else:
    ...
    def _parse_loop_block(self) -> pd.DataFrame:
        """Parse a loop (tabular) data block."""
        # parse loop header
        loop_column_names: deque[str] = deque()
        self.current_line_number += 1
        ...
        loop_data_lines: deque[str] = deque()
        while self.current_line_number <= self.n_lines_in_file:
            if self.current_line.startswith("data_"):
                break
            loop_data_lines.append(self.current_line)
            self.current_line_number += 1
        loop_data = "\n".join(loop_data_lines)
        if loop_data[-2:] != "\n":
            loop_data += "\n"
        ...
        if loop_data.startswith("\n"):
            ...
            df_numeric[df_numeric.columns[df_numeric.isna().all()]] = df[
                df_numeric.columns[df_numeric.isna().all()]
            ]

            # Replace columns that should be strings
            for col in df.columns:
                df[col] = (
                    df_numeric[col] if col not in self.parse_as_string else df[col]
                )
        return df
...
def block_name_from_line(line: str) -> str:
    """Extract block name from data line ('data_general' -> 'general')."""
    return line[5:]


def heading_from_line(line: str) -> str:
    """Extract col name from heading ('_rlnSpectralIndex #1' -> 'rlnSpectralIndex')."""
    return line.split()[0][1:]


def numericise(value: str) -> str | int | float:
    """Convert string value to numeric type if possible."""
    try:
...
def _apply_numeric(col: pd.Series) -> pd.Series:
    """Apply numeric conversion to a pandas Series."""
    try:
