...
from .constants import loop_simple, postprocess, two_single_line_loop_blocks
...
def test_round_trip_two_single_line_loop_blocks(tmp_path):
    ...
    # assert
    for expected_df, actual_df in zip(
        expected.values(), star_after_round_trip.values()
    ):
...
def test_dataframe_name_magic(tmp_path):
    ...
    assert (
        function_read.name == "particles"
    ), "Expected the name of the DataFrame to be 'particles'."
