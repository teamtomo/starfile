...
def test_instantiation(polars):
    ...
    with pytest.raises(FileNotFoundError):
        s = StarParser(non_existant_file, polars=polars)


@pytest.mark.parametrize("polars", [True, False])
def test_read_loop_block(polars):
    """
    Check that loop block is parsed correctly, data has the correct shape
    """
    parser = StarParser(loop_simple, polars=polars)
    ...
    assert isinstance(df, pl.DataFrame if polars else pd.DataFrame)
...
@pytest.mark.parametrize("polars", [True, False])
def test_read_multiblock_file(polars):
    """
    Check that multiblock STAR files such as postprocess RELION files
    parse properly
    """
    parser = StarParser(postprocess, polars=polars)
    ...
    assert isinstance(
        parser.data_blocks["fsc"], pl.DataFrame if polars else pd.DataFrame
    )
    assert parser.data_blocks["fsc"].shape == (49, 7)

    assert "guinier" in parser.data_blocks
    assert isinstance(
        parser.data_blocks["guinier"], pl.DataFrame if polars else pd.DataFrame
    )
    assert parser.data_blocks["guinier"].shape == (49, 3)


@pytest.mark.parametrize("polars", [True, False])
def test_read_pipeline(polars):
    """
    Check that a pipeline.star file is parsed correctly
    """
    parser = StarParser(pipeline, polars=polars)
...
@pytest.mark.parametrize("polars", [True, False])
def test_read_rln31(polars):
    """
    Check that reading of RELION 3.1 style star files works properly
    """
    s = StarParser(rln31_style, polars=polars)

    for key, df in s.data_blocks.items():
        assert isinstance(df, pl.DataFrame if polars else pd.DataFrame)

    assert isinstance(
        s.data_blocks["block_1"], pl.DataFrame if polars else pd.DataFrame
    )
    assert isinstance(
        s.data_blocks["block_2"], pl.DataFrame if polars else pd.DataFrame
    )
    assert isinstance(
        s.data_blocks["block_3"], pl.DataFrame if polars else pd.DataFrame
    )


@pytest.mark.parametrize("polars", [True, False])
def test_read_n_blocks(polars):
    """
    Check that passing read_n_blocks allows reading of only a specified
    number of data blocks from a star file
    """
    # test 1 block
    s = StarParser(postprocess, n_blocks_to_read=1, polars=polars)
    assert len(s.data_blocks) == 1

    # test 2 blocks
    s = StarParser(postprocess, n_blocks_to_read=2, polars=polars)
    assert len(s.data_blocks) == 2


@pytest.mark.parametrize("polars", [True, False])
def test_single_line_middle_of_multiblock(polars):
    s = StarParser(single_line_middle_of_multiblock, polars=polars)
    assert len(s.data_blocks) == 2


@pytest.mark.parametrize("polars", [True, False])
def test_single_line_end_of_multiblock(polars):
    s = StarParser(single_line_end_of_multiblock, polars=polars)
    assert len(s.data_blocks) == 2
...
@pytest.mark.parametrize("polars", [True, False])
def test_read_optimiser_2d(polars):
    parser = StarParser(optimiser_2d, polars=polars)
    assert len(parser.data_blocks) == 1
    assert len(parser.data_blocks["optimiser_general"]) == 84


@pytest.mark.parametrize("polars", [True, False])
def test_read_optimiser_3d(polars):
    parser = StarParser(optimiser_3d, polars=polars)
    assert len(parser.data_blocks) == 1
    assert len(parser.data_blocks["optimiser_general"]) == 84


@pytest.mark.parametrize("polars", [True, False])
def test_read_sampling_2d(polars):
    parser = StarParser(sampling_2d, polars=polars)
    assert len(parser.data_blocks) == 1
    assert len(parser.data_blocks["sampling_general"]) == 12


@pytest.mark.parametrize("polars", [True, False])
def test_read_sampling_3d(polars):
    parser = StarParser(sampling_3d, polars=polars)
...
@pytest.mark.parametrize("polars", [True, False])
def test_parsing_speed(polars):
    generate_large_star_file()
    start = time.time()
    s = StarParser(million_row_file, polars=polars)
    end = time.time()
...
@pytest.mark.parametrize("polars", [True, False])
def test_two_single_line_loop_blocks(polars):
    parser = StarParser(two_single_line_loop_blocks, polars=polars)
    assert len(parser.data_blocks) == 2
...
@pytest.mark.parametrize("polars", [True, False])
def test_two_basic_blocks(polars):
    parser = StarParser(two_basic_blocks, polars=polars)
    assert len(parser.data_blocks) == 2
...
@pytest.mark.parametrize("polars", [True, False])
def test_empty_loop_block(polars):
    """Parsing an empty loop block should return an empty dataframe."""
    parser = StarParser(empty_loop, polars=polars)
    assert len(parser.data_blocks) == 1
...
@pytest.mark.parametrize("polars", [True, False])
def test_parse_as_string(polars):
    parser = StarParser(
        postprocess,
        parse_as_string=["rlnFinalResolution", "rlnResolution"],
        polars=polars,
    )
    ...
    assert df["rlnResolution"].dtype == pl.String if polars else "object"
